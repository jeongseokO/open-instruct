...
def _get_prefill_lower_solo_bos_segmented_step_fn(model):
    candidates = []
    m = _unwrap_model(model)
    candidates.append(m)
    if hasattr(m, "base_model"):
    ...
    for cand in candidates:
        if cand is not None and hasattr(cand, "segmented_prefill_lower_solo_bos_step_logits"):
            return getattr(cand, "segmented_prefill_lower_solo_bos_step_logits")
    return None
...
def compute_prefill_lower_solo_bos_batch_loss(
    model,
    batch: dict[str, Any],
    *,
    lower_k: int,
    ...
    if (
        ...
        raise RuntimeError("Prefill-lower solo-BOS-attention training requires prebatched structured segment tensors.")

    step_fn = _get_prefill_lower_solo_bos_segmented_step_fn(model)
    if step_fn is None:
        raise RuntimeError("Prefill-lower solo-BOS-attention training requires segmented_prefill_lower_solo_bos_step_logits on the model.")
    out = step_fn(
        system_ids=system_ids.to(device=device),
        system_attention_mask=system_mask.to(device=device),
        user_ids=user_ids.to(device=device),
